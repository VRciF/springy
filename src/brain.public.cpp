...
Brain& Brain::init(){
    ...
    this->desc.add_options()
        ("help", "show help")
        ("o", "additional options like id, logfile, loglevel, fuse options")
...
Brain& Brain::setUp(int argc, char *argv[]){
    po::variables_map vm;
    po::store(po::parse_command_line(argc, argv, this->desc), vm);
    po::notify(vm);
...
Brain& Brain::run(){
    signals.async_wait(Brain::handler);
